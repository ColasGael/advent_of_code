...
def main(input_lines):
    PASSWORD_CHECK_PATTERN = re.compile("(?P<lower_bound>\d+)-(?P<upper_bound>\d+)\s(?P<target_char>[a-z]):\s(?P<password>[a-z]+)")
    ...
    for line in input_lines:
        ...
        password = (int(match.group("lower_bound")), int(match.group("upper_bound")), match.group("target_char"), match.group("password"))
...
def part2_is_valid_password(first_index, second_index, target_char, password):
    return (password[first_index-1] == target_char) ^ (password[second_index-1] == target_char)  # XOR
