def main(input_lines):
    diagnostic_report = [
        [int(bit) for bit in diagnostic_line] for diagnostic_line in input_lines
    ]

    gamma_rate, epsilon_rate = analyze_power_consumption(diagnostic_report)
    power_consumption = gamma_rate * epsilon_rate

    oxygen_generator_rating, co2_scrubber_rating = analyze_life_support(
        diagnostic_report
    )
...
def analyze_power_consumption(diagnostic_report):
    ...
    """
    n = len(diagnostic_report)  # pylint: disable=invalid-name
    m = len(diagnostic_report[0])  # pylint: disable=invalid-name
...
def analyze_life_support(diagnostic_report):
    ...
    for position in range(m):
        ...
        most_common_bit = find_most_common_bit(
            oxygen_generator_rating_candidates, position
        )
        oxygen_generator_rating_candidates = [
            rating_candidate
            for rating_candidate in oxygen_generator_rating_candidates
    ...
    for position in range(m):
        ...
        co2_scrubber_rating_candidates = [
            rating_candidate
            for rating_candidate in co2_scrubber_rating_candidates
