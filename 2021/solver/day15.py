...
def parse_map(input_lines):
    risk_levels = [[int(risk_level) for risk_level in row] for row in input_lines]
    return risk_levels
...
def extend_map(risk_levels, n_repeats=5):
    ...
    for i in range(n_repeats):
        for row_idx in range(  # pylint: disable=consider-using-enumerate
            len(risk_levels)
        ):
...
def a_star(risk_levels):
    ...
    while current_point != end_point:
        for neighbour in get_neighbours(
            current_point, len(risk_levels), len(risk_levels[0])
        ):
            tmp_g_score = g_scores[current_point] + distance(neighbour, risk_levels)
            ...
            f_scores[neighbour] = tmp_g_score + compute_h_score(
...
def distance(point_b, risk_levels):
