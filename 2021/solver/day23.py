...
def parse_burrow(input_lines):
    ...
    amphipod_types = {"A": 0, "B": 1, "C": 2, "D": 3}
    amphipod_counts = {amphipod_type: 0 for amphipod_type in amphipod_types}
    ...
    for i, input_line in enumerate(input_lines):
        ...
        if (i == 0):
            ...
            for room_id, c_idx in enumerate(range(1 + 2 * (i == 2), expected_len - 1, 2)):
                c = input_line[c_idx]
                if c not in amphipod_types:
                    is_expected_layout = False
                    break
                expected_line[c_idx] = c

                amphipod = Amphipod(amphipod_types[c], amphipod_counts[c])
...
def a_star(initial_state):
    ...
    while len(cost_to_goal_estimates) > 0:
        ...
        for cost, transition in current_state.get_potential_transitions():
            ...
            if (new_state not in cost_to_states) or (cost_to_states[new_state] > new_cost_to_state):
...
class Amphipod(object):
    def __init__(self, type, num):
        self.type = type
        self.num = num
        self.energy_per_step = 10 ** type

    def __repr__(self):
        return str((self.type, self.num))

    def __hash__(self):
        return hash((self.type, self.num))
...
class Burrow(object):
    ...
    def __hash__(self):
        ...
        for amphipod in self.hallway:
            if amphipod is not None:
                all_positions.append(amphipod.type)
            else:
    ...
    def add_amphipod(self, amphipod, position, in_hallway=True, room_id=None):
        if (not in_hallway) and (room_id is None):
            raise RuntimeError("A room id must be specified when adding an amphipod to a room")
    ...
    def move(self, amphipod, new_position, new_in_hallway=True, new_room_id=None):
        ...
        self.add_amphipod(amphipod, new_position, in_hallway=new_in_hallway, room_id=new_room_id)
    ...
    def is_final(self, amphipod):
        ...
        for other_amphipod in self.rooms[room_id][position + 1:]:
            if (other_amphipod.type != amphipod.type):
                return False
        return True
    ...
    def get_valid_transition(self, amphipod, in_hallway, new_position, n_steps):
        ...
        if new_position in self.positions_to_rooms:
            ...
            for other_position, other_amphipod in enumerate(room):
                ...
                elif (other_amphipod.type != amphipod.type):
                    return
            # room is full
    ...
    def get_potential_amphipod_transitions(self, amphipod):
        ...
        if (not in_hallway) and (position > 0) and (self.rooms[room_id][position - 1] is not None):
            return []
    ...
    def get_potential_transitions(self):
        ...
        for amphipod in self.amphipods_positions:
            if self.is_final(amphipod):
                continue
            for potential_transition in self.get_potential_amphipod_transitions(amphipod):
                n_steps, (position, in_hallway, room_id) = potential_transition
                cost = n_steps * amphipod.energy_per_step
                potential_transitions.append((cost, (amphipod, position, in_hallway, room_id)))
        return potential_transitions
    ...
    def cost_to_goal_heuristic(self):
        ...
        for amphipod, full_position in self.amphipods_positions.items():
            ...
            goal_hallway_position = self.rooms_to_positions[amphipod.type]
