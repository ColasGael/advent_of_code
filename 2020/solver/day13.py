def main(input_lines, unavailable_char="x"):
    ...
    available_buses = [
        ...
        if (bus_id != unavailable_char)
    ]

    earliest_bus_id = min(
        available_buses, key=lambda bus: get_next_bus_departure_ts(start_ts, bus[1])
    )[1]
    wait_time = get_next_bus_departure_ts(start_ts, earliest_bus_id) - start_ts
    part1_answer = earliest_bus_id * wait_time

    earliest_synchronized_departure_ts = find_synchronized_departure_ts_fast(
        available_buses
    )
...
def find_synchronized_departure_ts_naive(available_buses):
    ...
    while True:
        ...
        for bus_position, bus_id in available_buses:
            is_synchronized = (
                get_next_bus_departure_ts(start_ts, bus_id) == start_ts + bus_position
            )
