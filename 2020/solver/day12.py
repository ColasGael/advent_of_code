...
INSTRUCTION_PATTERN = re.compile(
    r"(?P<action>[{}])(?P<value>\d+)".format("".join(SUPPORTED_ACTIONS))
)
...
def main(
    ...
    for line in input_lines:
        ...
        actions.append((match.group("action"), int(match.group("value"))))
    ...
    for action, value in actions:
        part1_position, part1_orientation = part1_apply_action(
            action, value, CARDINAL_POINTS, part1_position, part1_orientation
        )
        part2_position, part2_orientation, waypoint_position = part2_apply_action(
            action,
            value,
            CARDINAL_POINTS,
            part2_position,
...
def part2_apply_action(  # pylint: disable=too-many-arguments
    ...
    if action == cardinal_points[0]:
        ...
        position = (
            position[0] + waypoint_position[0] * value,
            position[1] + waypoint_position[1] * value,
        )
