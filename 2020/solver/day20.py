...
def main(input_lines, pixel_0=".", pixel_1="#", visualize=False):
    tiles = parse_input(input_lines, pixel_1)
    puzzle = solve_puzzle(tiles)
    part1_answer = np.prod([puzzle[i, j] for i in (0, -1) for j in (0, -1)])

    final_image = merge_tiles(tiles, puzzle)
    monster_image = parse_tile(MONSTER_PATTERN, pixel_1)
    if visualize:
        visualize_tile(apply_rotation(np.flipud(final_image), 90), pixel_0, pixel_1)
    n_monsters = find_monsters(final_image, monster_image)
    part2_answer = np.sum(final_image) - n_monsters * np.sum(monster_image)
    return part1_answer, part2_answer


def parse_input(input_lines, pixel_1):
    tiles = {}
    tile_raw = []
    for input_line in input_lines:
        if len(input_line) == 0:
            tiles[tile_id] = parse_tile(tile_raw, pixel_1)
            tile_raw = []
        elif "Tile" in input_line:
            tile_id = int(re.search(r"\d+", input_line).group())
        else:
...
def parse_tile(tile_raw, pixel_1):
    tile_array = np.array(
        [[char == pixel_1 for char in tile_line] for tile_line in tile_raw]
    )
    return tile_array
...
def solve_puzzle(tiles):
    ...
    if not check_assumptions(connections):
        raise RuntimeError(
            "This input cannot be solved with this method: "
            "1 or more of the tile matches with another it is not connected with!"
        )

    orientations = {
        this_tile_id: {
            this_orientation: other_tile_id
            for other_tile_id, this_orientation in this_tile_connections.items()
        }
    ...
    first_corner_id = [
        tile_id for tile_id in connections.keys() if (len(connections[tile_id]) == 2)
    ][0]
    ...
    if not align_rotation(last_down_orientation, last_right_orientation) == 90:
        last_down_orientation, last_right_orientation = (
            last_right_orientation,
            last_down_orientation,
        )

    for i in range(side_length):
        if i != 0:
            puzzle[i, 0] = orientations[puzzle[i - 1, 0]][last_down_orientation]
            last_down_orientation = align_rotation(
                connections[puzzle[i, 0]][puzzle[i - 1, 0]], 180
            )
            last_right_orientation = align_rotation(last_down_orientation, 90)
        tiles[puzzle[i, 0]] = apply_rotation(
            tiles[puzzle[i, 0]], align_rotation(last_down_orientation, 180)
        )
        for j in range(1, side_length):
            puzzle[i, j] = orientations[puzzle[i, j - 1]][last_right_orientation]
            last_right_orientation = align_rotation(
                connections[puzzle[i, j]][puzzle[i, j - 1]], 180
            )
            tiles[puzzle[i, j]] = apply_rotation(
                tiles[puzzle[i, j]], align_rotation(last_right_orientation, 90)
            )
    return puzzle
    ...
    while todo_tiles:
        ...
        for other_tile_id, other_tile_borders in borders.items():
            ...
            for (
                this_is_flipped,
                this_rotation,
            ), this_tile_border in this_tile_borders.items():
                if this_is_flipped:
                    continue
                for (
                    other_is_flipped,
                    other_rotation,
                ), other_tile_border in other_tile_borders.items():
                    if np.all(this_tile_border == other_tile_border[::-1]):
                        ...
                        if other_is_flipped:
                            ...
                            borders[other_tile_id] = {
                                (not is_flipped, rotation): tile_border
                                for (is_flipped, rotation), tile_border in borders[
                                    other_tile_id
                                ].items()
                            }
...
def get_borders(tile):
    # (False, 0): tile[0, :], (False, 90): np.flip(tile[:, 0]),
    # (False, 180): np.flip(tile[-1, :]), (False, 270): tile[:, -1],
    # (True, 0): tile[-1, :], (True, 90): tile[:, 0],
    # (True, 180): np.flip(tile[0, :]), (True, 270): np.flip(tile[:, -1]),
    borders = {
        (is_flipped, rotation_angle): apply_rotation(
            np.flipud(tile) if is_flipped else tile, rotation_angle
        )[0, :]
...
def visualize_tile(tile, pixel_0, pixel_1):
    image_viz = ""
...
def find_monsters(image, monster_image):
    ...
    n_monsters = 0
    oriented_images = [
        apply_rotation(np.flipud(image) if is_flipped else image, rotation_angle)
        for is_flipped in (False, True)
        for rotation_angle in (0, 90, 180, 270)
    ]
    for oriented_image in oriented_images:
        matched_image = convolution_2d(oriented_image, monster_image)
