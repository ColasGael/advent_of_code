...
def main(input_lines, target_expense=2020):
    ...
    part1_answer = np.prod(find_sum_of_2(target_expense, expenses))
    part2_answer = np.prod(find_sum_of_3(target_expense, expenses))
    return part1_answer, part2_answer


def find_sum_of_2(target_expense, expenses, allow_failure=False):
    # Complexity: N^2
    ...
    if not allow_failure:
        raise RuntimeError("Could not find valid sum of 2")
    return None


def find_sum_of_3(target_expense, expenses):
    # Complexity: N^3
    # for i, this_expense in enumerate(expenses):
    ...
    for i, expense in enumerate(expenses):
        ...
        result = find_sum_of_2(
            revised_target_expense, expenses[i + 1 :], allow_failure=True
        )
        if result:
            return (result[0], result[1], expense)
    raise RuntimeError("Could not find valid sum of 3")
