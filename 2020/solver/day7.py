...
def main(input_lines, target_bag="shiny gold"):
    BAG_RULE_PATTERN = re.compile("(?P<quantity>\d+) (?P<color>[a-z]+ [a-z]+) bag")
    ...
    for line in input_lines:
        ...
        downward_inheritance[parent_bag] = [(child_bag, int(quantity)) for (quantity, child_bag) in child_bags]
...
def get_upward_inheritance(downward_inheritance):
    ...
    for parent_bag, child_bags in downward_inheritance.items():
        for (child_bag, quantity) in child_bags:
...
def find_all_parents(child, upward_inheritance, parents={}):
    ...
    for direct_parent in direct_parents:
        if direct_parent not in parents:
            parents[direct_parent] = find_all_parents(direct_parent, upward_inheritance, parents=parents)
        all_parents = all_parents.union(parents[direct_parent])
    return all_parents


def find_number_children(parent, downward_inheritance, children={}):
    n_children = 0
    for (direct_child, quantity) in downward_inheritance[parent]:
        if direct_child not in children:
            children[direct_child] = find_number_children(direct_child, downward_inheritance, children=children)
