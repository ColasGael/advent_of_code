...
REQUIRED_FIELDS = {
    ...
    "hgt": lambda entry: ((150 <= int(entry[:-2]) <= 193) and (entry[-2:] == "cm"))
    or ((59 <= int(entry[:-2]) <= 76) and (entry[-2:] == "in")),
    "hcl": lambda entry: (len(entry) == 7)
    and (entry[0] == "#")
    and np.all(
        [
            char
...
def main(input_lines):
    ...
    for line in input_lines:
        ...
        if not passport_entries:
            ...
            passports[-1].update(passport_entries)

    part1_n_valid_passport = 0
    part2_n_valid_passport = 0
    for passport in passports:
        if is_valid_passport_fields(passport, REQUIRED_FIELDS.keys()):
            part1_n_valid_passport += 1
        if is_valid_passport_data(passport, REQUIRED_FIELDS):
            part2_n_valid_passport += 1

    return part1_n_valid_passport, part2_n_valid_passport


def is_valid_passport_fields(passport, required_fields):
...
def is_valid_passport_data(passport, required_fields):
    for required_field, data_validation_rule in required_fields.items():
        if (required_field not in passport) or not data_validation_rule(
            passport[required_field]
        ):
            return False
    return True
