...
def main(input_lines):
    INSTRUCTION_PATTERN = re.compile("(?P<operation>[a-z][a-z][a-z]) (?P<argument>[+-]\d+)")

    instructions = []
    for line in input_lines:
        m = INSTRUCTION_PATTERN.match(line)
        instructions.append((m.group('operation'), int(m.group('argument'))))
...
def check_infinite_loop(instructions, i=0, acc=0, is_visited_before=None):
    if is_visited_before is not None:
        is_visited = [x for x in is_visited_before]
    else:
...
def fix_infinite_loop(instructions):
    ...
    while 0 <= i < len(instructions):
        ...
        j, _acc = check_infinite_loop(instructions, i=i, acc=acc, is_visited_before=is_visited)
