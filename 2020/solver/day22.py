...
def main(input_lines):
    ...
    recursive_combat_winner_deck = play_combat(player_1_deck, player_2_deck, play_recursive_round)[1]
...
def parse_input(input_lines, n_players=2):
    PLAYER_PATTERN = re.compile("Player (?P<player_no>\d+):")
    ...
    for input_line in input_lines:
        ...
        if m:
            current_player_no = int(m.group('player_no')) - 1
        else:
...
def play_combat(player_1_deck, player_2_deck, play_round_method):
    ...
    while (not is_game_won_by_player_1) and (len(player_1_deck) != 0) and (len(player_2_deck) != 0):
...
def hash_configuration(player_1_deck, player_2_deck):
    configuration_hash = "1#" + "".join([str(player_1_card) for player_1_card in player_1_deck]) + "2#" + "".join([str(player_2_card) for player_2_card in player_2_deck])
...
def play_recursive_round(player_1_deck, player_2_deck):
    ...
    if (len(player_1_deck) >= player_1_card) and (len(player_2_deck) >= player_2_card):
        ...
        is_round_won_by_player_1 = play_combat(player_1_subdeck, player_2_subdeck, play_recursive_round)[0]
    else:
